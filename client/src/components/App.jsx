...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(`http://localhost:3002/api/item/${enteredId}`)
  ...
  render() {
    ...
    var wrapperStyle = {
      ...
      position: 'absolute',
      left: '885px',
      borderLeft: 'solid 0.5px #dfdfdf',
      borderRight: 'solid 0.5px #dfdfdf',
      padding: '15px',
    }
        ...
        <div>
          ...
          <div style={{ borderBottom: 'solid #dcdcdc' }}>
            ...
            <span style={questionStyle}>
              {this.state.items.totalQuestions} Questions
            </span>
          </div>
          ...
          <div style={descriptionStyle}>
            Color{' '}
            <span style={{ fontFamily: 'Arial' }}>{this.state.displayColor}</span>
            <div style={{ textAlign: 'center' }}>
              {this.state.items &&
                this.state.items.colors &&
                this.state.items.colors.map(color => (
                  <Color
                    key={color}
                    color={color}
          ...
          <div style={descriptionStyle}>
            Size{' '}
            <span style={{ fontFamily: 'Arial' }}>{this.state.displaySize}</span>
            <div style={{ textAlign: 'center' }}>
              {this.state.items &&
                this.state.items.sizes &&
                this.state.items.sizes.map(size => (
                  <Size key={size} size={size} onClickSize={this.onClickSize} />
                ))}
            </div>
          </div>
          <Quantity quantity={this.state.items.quantityCanBuy} />
